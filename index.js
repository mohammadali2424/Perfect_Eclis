...
// مسیر اصلی برای رفع ارور Cannot GET
app.get('/', (req, res) => {
...
// 🔥 اضافه کردن تست اتصال به تلگرام
async function testTelegramConnection() {
  try {
    const botInfo = await bot.telegram.getMe();
    console.log('✅ اتصال به تلگرام موفقیت‌آمیز بود. اطلاعات ربات:', botInfo);
    return true;
  } catch (error) {
...
// 🔥 اضافه کردن تست اتصال به Supabase
async function testSupabaseConnection() {
  try {
    ...
    console.error('❌ خطای غیرمنتظره در اتصال به Supabase:', error.message);
    return false;
  }
}

// 🔥 اجرای تست‌های اتصال هنگام راه‌اندازی
async function startupTests() {
  console.log('🔍 در حال اجرای تست‌های راه‌اندازی...');
  
  const telegramTest = await testTelegramConnection();
  const supabaseTest = await testSupabaseConnection();
  
  if (!telegramTest) {
    console.error('❌ ربات نمی‌تواند به تلگرام متصل شود. ممکن است توکن نامعتبر باشد یا سرور به اینترنت دسترسی نداشته باشد.');
  }
  
  if (!supabaseTest) {
    console.error('❌ ربات نمی‌تواند به Supabase متصل شود. لطفاً URL و کلید را بررسی کنید.');
  }
  
  if (telegramTest && supabaseTest) {
    console.log('✅ همه تست‌های اتصال موفقیت‌آمیز بودند.');
  }
}
...
// 🔥 شروع ربات با Long Polling به جای Webhook
// اگر از رندر استفاده می‌کنید، ممکن است نیاز باشد از Long Polling استفاده کنید
// زیرا رندر ممکن است با Webhook مشکلاتی داشته باشد

// اگر می‌خواهید از Long Polling استفاده کنید، این خط را فعال کنید و قسمت وب‌هوک را غیرفعال کنید
// bot.launch().then(() => {
...
// اگر می‌خواهید از Webhook استفاده کنید، این خط را فعال کنید
// اما مطمئن شوید که وب‌هوک به درستی تنظیم شده است

// در عوض، از راه‌اندازی خودکار استفاده می‌کنیم که هم Webhook و هم Long Polling را پشتیبانی کند
// اگر متغیر محیطی WEBHOOK_URL تنظیم شده باشد از Webhook استفاده می‌کند، در غیر این صورت از Long Polling
