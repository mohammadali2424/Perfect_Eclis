...
// 🔥 تصحیح دستور start - این بخش مشکل اصلی را حل می‌کند
bot.start(async (ctx) => {
  try {
...
bot.command('scan_members', async (ctx) => {
  if (!BOT_OWNERS.includes(ctx.from.id)) {
    return ctx.reply('❌ فقط مالک ربات می‌تواند ازこの دستور استفاده کند.');
  }
...
// 🔥 بخش حیاتی: راه‌اندازی سرور و وب‌هوک برای Render
...
app.get('/', (req, res) => {
  res.json({ status: 'ok', message: 'ربات تلگرام در حال اجراست' });
});
...
app.post('/webhook', async (req, res) => {
  try {
    ...
    res.status(200).send('OK'); // باز هم OK برگردانید حتی اگر خطا وجود دارد
  }
});
...
app.listen(PORT, async () => {
  ...
  // فقط در محیط production از webhook استفاده کنید
  if (process.env.NODE_ENV === 'production') {
    ...
    bot.launch();
    console.log('🤖 ربات در حالت توسعه (polling) راه‌اندازی شد');
  }
