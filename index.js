...
const requiredEnvVars = ['BOT_TOKEN', 'SUPABASE_URL', 'SUPABASE_KEY', 'RENDER_EXTERNAL_URL'];
...
bot.hears(/.*#خروج.*/, async (ctx) => {
  try {
    ...
    if (updateError) {
      ...
      return ctx.reply('❌ خطا در به روز رس��نی وضعیت قرنطینه.');
    }
...
app.listen(PORT, async () => {
  ...
  try {
    ...
    console.log(`✅ وب‌هوک با موفقیت تنظیم شد: ${webhookUrl}`);
    console.log('🤖 ربات در حالت webhook راه‌اندازی شد');
    
    // 🔥 اطلاع به مالک درباره راه‌اندازی
    await notifyOwner('✅ ربات با موفقیت راه‌اندازی شد و در حال اجراست');
...
process.once('SIGINT', () => {
  console.log('🛑 دریافت سیگنال SIGINT، خاموش کردن...');
  process.exit(0);
});

process.once('SIGTERM', () => {
  console.log('🛑 دریافت سیگنال SIGTERM، خاموش کردن...');
  process.exit(0);
});

// 🔥 مدیریت خطاهای全局
bot.catch((err, ctx) => {
  console.error(`خطای全局 برای آپدیت ${ctx.updateType}:`, err);
});

console.log('✅ ماژول‌ها با موفقیت بارگذاری شدند');