...
bot.command('test_quarantine', async (ctx) => {
  ...
  try {
    const result = await autoQuarantineUser(
      ctx.from.id,
      ctx.chat.id,
      {
        first_name: ctx.from.first_name,
...
// 🔥 تابع بررسی و به‌روزرسانی وضعیت گروه‌ها
async function updateAllGroupsStatus() {
  try {
    const { data: groups, error } = await supabase
      .from('groups')
    ...
    for (const group of groups) {
      try {
        const isAdmin = await checkBotAdminStatus(group.chat_id);
        console.log(`✅ وضعیت گروه ${group.title}: ${isAdmin ? 'ادمین' : 'غیر ادمین'}`);
      } catch (error) {
        console.error(`Error updating group ${group.chat_id}:`, error);
      }
    }
  } catch (error) {
    console.error('Error in updateAllGroupsStatus:', error);
  }
}

// زمان‌بندی برای به‌روزرسانی روزانه وضعیت گروه‌ها
setInterval(updateAllGroupsStatus, 24 * 60 * 60 * 1000);
