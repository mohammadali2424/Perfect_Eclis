...
// تنظیم مالکین ربات - اینجا آی دی مالکین را قرار دهید
...
// تابع برای بررسی دسترسی کاربر
function checkUserAccess(ctx, requiredLevel = 'user') {
  ...
  // دسترسی ادمین‌های گروه
  if (chatType === 'group' || chatType === 'supergroup') {
    if (requiredLevel === 'admin') {
      return ctx.from.status === 'administrator' || ctx.from.status === 'creator';
    }
  }
  
  return requiredLevel === 'user';
}
...
// 🔥 بهبود یافته: هندلر ثبت گروه با بروزرسانی فوری
bot.on('my_chat_member', async (ctx) => {
  try {
    ...
    if (chatType === 'group' || chatType === 'supergroup') {
      ...
      if (!error) {
        console.log(`✅ گروه جدید ثبت شد: ${chatTitle} (${chatId}) - وضعیت ادمین: ${isBotAdmin}`);
        
        // 🔥 پاکسازی کش و بررسی مجدد وضعیت
        userCache.delete(`bot_admin_${chatId}`);
        await checkBotAdminStatus(chatId);
      }
    }
  } catch (error) {
...
// 🔥 استفاده در هندلر کاربران جدید
bot.on('new_chat_members', async (ctx) => {
  try {
    const chatId = ctx.chat.id;
    
    // 🔥 بررسی با تأخیر برای اطمینان از ثبت گروه
...
// 🔥 کاهش زمان کش برای گروه‌های جدید
async function checkBotAdminStatus(chatId) {
  try {
    const cacheKey = `bot_admin_${chatId}`;
...
bot.command('set_trigger', (ctx) => {
  if (!checkUserAccess(ctx, 'admin')) {
    return ctx.reply('❌ فقط مالک و ادمین‌ها می‌توانند از این دستور استفاده کنند.');
  }
