...
// 🔥 بهبود اساسی: هندلر تقویت شده برای کاربران جدید
bot.on('new_chat_members', async (ctx) => {
  try {
    ...
    for (const newMember of ctx.message.new_chat_members) {
      ...
      console.log(`🔍 کاربر جدید شناسایی شد: ${userId} در گروه ${chatId}`);
      ...
      if (quarantine) {
        ...
        if (quarantine.chat_id !== chatId) {
          // کاربر در قرنطینه گروه دیگری است، پس از این گروه کیک می‌شود
          console.log(`🚫 کاربر ${userId} در قرنطینه گروه ${quarantine.chat_id} است، از گروه ${chatId} کیک می‌شود`);
          await kickUserFromGroup(chatId, userId, 'کاربر در قرنطینه است');
        } else {
          // کاربر در قرنطینه همین گروه است، کاری لازم نیست
          console.log(`✅ کاربر ${userId} در قرنطینه همین گروه است`);
        }
        continue;
      }
      
      // کاربر جدید، قرنطینه می‌شود
      console.log(`🔒 کاربر جدید ${userId} قرنطینه می‌شود`);
      ...
      if (quarantineError) {
        console.error('Error setting user quarantine:', quarantineError);
        continue;
      }

      // پاکسازی کش و کیک از گروه‌های دیگر
      userCache.delete(`quarantine_${userId}`);
      const kickedCount = await kickUserFromAllGroupsExceptCurrent(userId, chatId);
      
      console.log(`✅ کاربر ${userId} در گروه جدید ${chatTitle} (${chatId}) قرنطینه شد و از ${kickedCount} گروه دیگر کیک شد`);
    }
  } catch (error) {
...
// 🔥 بهبود اساسی: هندلر برای بررسی کاربران قرنطینه هنگام تغییر وضعیت
bot.on('chat_member', async (ctx) => {
  try {
    ...
    if (newMember.status === 'member' || newMember.status === 'administrator') {
      console.log(`🔍 تغییر وضعیت کاربر شناسایی شد: ${userId} در گروه ${chatId}`);
      
      // بررسی آیا کاربر در قرنطینه است
      const quarantine = await checkUserQuarantine(userId);
      
      if (quarantine && quarantine.chat_id !== chatId) {
        console.log(`🚫 کاربر ${userId} در قرنطینه گروه دیگری است، از گروه ${chatId} کیک می‌شود`);
        ...
        if (isBotAdmin) {
          ...
          console.log(`⚠️ ربات در گروه ${chatId} ادمین نیست، نمی‌تواند کاربر را کیک کند`);
        }
      }
    }
  } catch (error) {
