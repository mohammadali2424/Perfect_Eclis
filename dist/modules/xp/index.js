function parseUserRef(token) {
    // Accept @username or numeric id
    const t = token.trim();
    if (/^@/.test(t))
        return { username: t.slice(1) };
    const num = Number(t);
...
async function resolveTarget(ctx, args) {
    ...
    if (ref.telegramId)
        return { id: ref.telegramId, label: String(ref.telegramId) };
...
export function registerXpModule(registry) {
    registry.register({
        name: 'xp',
        ...
        handler: async (ctx, { uow, logger }) => {
            ...
            if (sub === 'add' || sub === 'give' || sub === 'افزودن' || sub === 'دادن') {
                ...
                const before = await uow.players.getOrCreateFromTelegram({ id: target.id, first_name: target.label });
            ...
            if (sub === 'report' || sub === 'گزارش') {
                ...
                const entries = await uow.xp.listByChat(chatId, { limit: 30 });
                const sum = entries.reduce((a, e) => a + e.amount, 0);
                await ctx.reply([`گزارش XP این چت (آخرین ${entries.length} رویداد):`, `جمع: ${sum}`, '', ...entries.map((e) => `• ${e.playerId}: ${e.amount > 0 ? '+' : ''}${e.amount} — ${e.reason}`)].join('\n'));
                return;
            }
