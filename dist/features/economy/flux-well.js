...
exports.registerFluxWellFeature = registerFluxWellFeature;
// src/features/economy/flux-well.ts
const grammy_1 = require("grammy");
function clampInt(n, min, max) {
    return Math.max(min, Math.min(max, Math.floor(n)));
}
async function getBankChatId(ctx) {
    var _a;
    ...
    return (_a = data === null || data === void 0 ? void 0 : data.bank_chat_id) !== null && _a !== void 0 ? _a : null;
}
async function getFluxPricePerPercent(ctx) {
    var _a;
    const { supabase } = ctx.services;
    const { data, error } = await supabase
    ...
    const v = Number((_a = data === null || data === void 0 ? void 0 : data.value_json) === null || _a === void 0 ? void 0 : _a.per_percent);
    return Number.isFinite(v) && v > 0 ? v : null;
}
// ✅ در پروژه تو: چاه فعال یعنی رکورد وجود دارد (نه enabled)
async function hasWell(ctx, spotId, kind = "normal") {
    const { supabase } = ctx.services;
...
async function getCharAndVehicle(ctx) {
    const { supabase } = ctx.services;
    const { data: char, error: charErr } = await supabase
        ...
        .maybeSingle();
    if (charErr || !char)
        return { char: null, vehicle: null };
    if (!char.riding_vehicle_id)
        return { char, vehicle: null };
...
async function ensureStationRow(supabase, spotId) {
    await supabase
        .from("flux_station_state")
        .upsert({ spot_id: spotId, updated_at: new Date().toISOString() }, { onConflict: "spot_id" });
}
async function enterStation(supabase, spotId, userTgId) {
    ...
    const { data, error } = await supabase
        ...
        .maybeSingle();
    if (error || !data)
        throw error !== null && error !== void 0 ? error : new Error("station_state not found");
    const active = Array.isArray(data.active_user_tg_ids) ? data.active_user_tg_ids : [];
    const queue = Array.isArray(data.queue_user_tg_ids) ? data.queue_user_tg_ids : [];
    if (active.includes(userTgId))
        return { status: "ACTIVE", active, queue };
    if (queue.includes(userTgId))
        return { status: "QUEUED", active, queue };
    if (active.length < 2) {
...
async function leaveStation(supabase, spotId, userTgId) {
    const { data, error } = await supabase
        .from("flux_station_state")
        .select("*")
        .eq("spot_id", spotId)
    ...
    let active = Array.isArray(data.active_user_tg_ids) ? data.active_user_tg_ids : [];
    let queue = Array.isArray(data.queue_user_tg_ids) ? data.queue_user_tg_ids : [];
    ...
    const { error: updErr } = await supabase
        ...
        .eq("spot_id", spotId);
    if (updErr)
        console.error("leaveStation update error:", updErr);
    return { nextUser };
}
// --- Refuel sessions ---
async function createOrReplaceSession(ctx, spotId, userTgId, charId, vehicleId, currentFuel, targetFuel) {
    const { supabase } = ctx.services;
    ...
    const { data, error } = await supabase
        ...
        .maybeSingle();
    if (error || !data)
        throw error !== null && error !== void 0 ? error : new Error("create session failed");
    return data;
}
async function paySession(ctx, sessionId) {
    var _a;
    const { supabase } = ctx.services;
    ...
    if (bankChatId) {
        try {
            await ctx.api.sendMessage(bankChatId, "🏦 تراکنش سوخت‌گیری فلوکس\n" +
                `👤 کاربر: ${(_a = ctx.from) === null || _a === void 0 ? void 0 : _a.id}\n` +
                `📍 spot_id: ${s.spot_id}\n` +
    ...
    await ctx.reply(`✅ سوخت‌گیری انجام شد.\n` +
    ...
    if (nextUser) {
        try {
            await ctx.api.sendMessage(nextUser, `نوبتت رسید ✅\nبرای سوخت‌گیری دوباره روی «سوخت‌گیری» بزن.`);
        }
        catch { }
    }
}
function registerFluxWellFeature(bot) {
    // شروع سوخت‌گیری
    bot.callbackQuery("flux:refuel", async (ctx) => {
        ...
        if (!ctx.from)
            return;
        const { char, vehicle } = await getCharAndVehicle(ctx);
        if (!char || !vehicle) {
            await ctx.reply("برای سوخت‌گیری باید راننده‌ی ماشین خودت باشی.");
            return;
        }
        const spotId = Number((_a = char.current_spot_id) !== null && _a !== void 0 ? _a : 0);
        if (!spotId) {
            await ctx.reply("مکان فعلی مشخص نیست.");
            return;
        }
        // ✅ چاه فعال یعنی رکورد normal وجود دارد
        const ok = await hasWell(ctx, spotId, "normal");
        if (!ok) {
        ...
        await ctx.reply(`⛽ سوخت فعلی ماشینت: ${Number((_b = vehicle.fuel_percent) !== null && _b !== void 0 ? _b : 0).toFixed(1)}٪\n` +
            `تا چند درصد پر کنم؟`, { reply_markup: kb });
    });
    // انتخاب درصد هدف
    bot.callbackQuery(/^flux:refuel:set:(\d+)$/, async (ctx) => {
        ...
        const currentFuel = Number((_b = vehicle.fuel_percent) !== null && _b !== void 0 ? _b : 0);
        ...
        let session;
        try {
            session = await createOrReplaceSession(ctx, spotId, ctx.from.id, char.id, vehicle.id, currentFuel, target);
        }
        catch (e) {
            const msg = String((_c = e === null || e === void 0 ? void 0 : e.message) !== null && _c !== void 0 ? _c : e);
        ...
        const kb = new grammy_1.InlineKeyboard()
            ...
            .row()
            .text("❌ انصراف", "flux:refuel:cancel");
        await ctx.reply(`🧾 تایید سوخت‌گیری\n` +
    ...
    bot.callbackQuery(/^flux:refuel:confirm:([0-9a-f-]+)$/, async (ctx) => {
        await ctx.answerCallbackQuery();
        await paySession(ctx, ctx.match[1]);
    });
    // انصراف
    bot.callbackQuery("flux:refuel:cancel", async (ctx) => {
        ...
        const { data: char } = await supabase
            ...
            .eq("tg_id", ctx.from.id)
            .maybeSingle();
        if (char === null || char === void 0 ? void 0 : char.current_spot_id) {
