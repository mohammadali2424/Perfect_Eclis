...
exports.getShopChatId = getShopChatId;
exports.getBankChatId = getBankChatId;
exports.registerWorldVehicleShop = registerWorldVehicleShop;
...
// این‌ها را export می‌کنیم تا سایر فیچرها (مثل سوخت‌گیری/بانک) هم استفاده کنند.
async function getShopChatId(ctx) {
    var _a;
    ...
    return (_a = data === null || data === void 0 ? void 0 : data.shop_chat_id) !== null && _a !== void 0 ? _a : null;
}
...
async function isShopAdminOrMaster(ctx) {
    if (isMaster(ctx))
        return true;
    const { supabase } = ctx.services;
    if (!ctx.from)
...
function registerWorldVehicleShop(bot) {
    ...
    bot.hears("ثبت گروه بانک", async (ctx) => {
        ...
        await ctx.reply("🏦 این گروه به عنوان «گروه مدیریت بانک اکلیس» ثبت شد.\n" +
    ...
    bot.hears("حذف گروه بانک", async (ctx) => {
        ...
        await ctx.reply("🏦 این گروه دیگر به عنوان گروه بانک شناخته نمی‌شود.\n" +
    ...
    bot.hears("ثبت گروه شاپ", async (ctx) => {
        ...
        await ctx.reply("🛒 این گروه به عنوان «گروه شاپ وسایل نقلیه» ثبت شد.\n" +
    ...
    bot.hears("ثبت ادمین شاپ", async (ctx) => {
        ...
        const reply = (_a = ctx.message) === null || _a === void 0 ? void 0 : _a.reply_to_message;
        if (!reply || !reply.from) {
        ...
        await ctx.reply(`✅ کاربر [${reply.from.first_name}] به عنوان ادمین شاپ ثبت شد.`);
    });
    bot.hears("حذف ادمین شاپ", async (ctx) => {
        ...
        await ctx.reply(`✅ کاربر [${reply.from.first_name}] از ادمین‌های شاپ حذف شد.`);
    });
    //
    ...
    bot.hears("ثبت سراسری قیمت فلوکس", async (ctx) => {
        ...
        await ctx.reply("🧪 ثبت سراسری قیمت فلوکس\n" +
    ...
    bot.hears("ثبت وسیله", async (ctx) => {
        ...
        await ctx.reply("🚗 ثبت وسیله جدید\n" +
    ...
    bot.on("message:text", async (ctx, next) => {
        ...
        if (s.awaitingFluxPrice && isMaster(ctx)) {
            ...
            const { error } = await supabase
                ...
                .upsert({ key: "flux_base_price", value_json: { per_percent: value } }, { onConflict: "key" });
            if (error) {
            ...
            await ctx.reply(`✅ قیمت پایه فلوکس روی ${value} Solen برای هر ۱٪ باک تنظیم شد.`);
            return;
        }
        // ویزارد ثبت وسیله
        if (!s.vehicleWizard) {
        ...
        if (step === "ask_char_code") {
            ...
            if (!char) {
                await ctx.reply("چنین کاراکتری پیدا نشد. مطمئنی char_code را درست نوشتی؟");
                return;
            }
            ...
            w.step = "ask_type";
            await ctx.reply(`کاراکتر پیدا شد:\n` +
                `• نام: ${char.char_name || "—"}\n` +
                `• کد: ${char.char_code}\n` +
        ...
        if (step === "ask_type") {
            w.vehicleType = text;
            w.step = "ask_capacity";
            await ctx.reply("ظرفیت سرنشین را بنویس (چند نفر می‌توانند سوار شوند؟ عدد صحیح).");
            return;
        }
        ...
        if (step === "ask_title") {
            w.title = text;
            w.step = "confirm";
            await ctx.reply("🧾 خلاصه اطلاعات وسیله:\n" +
                `صاحب:\n` +
        ...
        if (step === "confirm") {
            ...
            await ctx.reply("✅ وسیله با موفقیت ثبت شد.\n" +
