...
exports.registerWorldAdminFeature = registerWorldAdminFeature;
const grammy_1 = require("grammy");
const config_1 = require("../../core/config");
function clanLabel(key) {
    switch (key) {
        case "walker":
            return "⚡ 𝐖𝐚𝐥𝐤𝐞𝐫";
        case "stellarieth":
...
async function sendScreen(ctx, text, kb) {
    var _a;
...
function regionPanelKeyboard(regionId, hasClan) {
...
async function showRegionLockMenu(ctx, regionId) {
...
async function showEdgeLockMenu(ctx, regionId) {
    const { supabase } = ctx.services;
    ...
    const related = edges.filter((e) => spotIds.includes(e.from_spot_id) ||
        spotIds.includes(e.to_spot_id));
    if (related.length === 0) {
    ...
    for (const e of related) {
        ...
        const baseLabel = `${(fromSpot === null || fromSpot === void 0 ? void 0 : fromSpot.title) || e.from_spot_id} → ${(toSpot === null || toSpot === void 0 ? void 0 : toSpot.title) || e.to_spot_id} (${e.travel_seconds}ث)`;
...
function registerWorldAdminFeature(bot) {
    ...
    bot.command("worldadmin", async (ctx) => {
        ...
        try {
            ...
            await ctx.api.sendMessage(config_1.MASTER_ID, "🧩 پنل جهان‌ساز برای این Region:\n\n" +
                `نام: ${title}\n` +
                `chat_id: ${chatId}\n` +
                `region_id: ${regionId}\n` +
                `خاندان: ${clanName || "هنوز تعیین نشده"}\n\n` +
                "با دکمه‌ها می‌توانی Spot بسازی، Edge تعریف کنی و جهان را شکل بدهی.", { reply_markup: kb });
        }
        catch (e) {
    ...
    bot.hears("قفل", async (ctx) => {
        ...
        const { data: region, error } = await supabase
            ...
            .maybeSingle();
        if (error || !region) {
            await ctx.api.sendMessage(config_1.MASTER_ID, "🔐 دستور «قفل» در گروهی زده شد که هنوز به عنوان Region ثبت نشده.\n" +
                "اول در آن گروه /worldadmin بزن.");
            return;
        }
        await ctx.api.sendMessage(config_1.MASTER_ID, "🔐 مدیریت قفل برای Region:\n" +
            `نام: ${region.title}\n` +
            `region_id: ${region.id}`, {
    ...
    bot.on("callback_query:data", async (ctx, next) => {
        ...
        if ((isAdminAction || isLockAction) && (!ctx.from || ctx.from.id !== config_1.MASTER_ID)) {
        ...
        if (data.startsWith("admin:setclan2:")) {
            const parts = data.split(":");
            const regionId = Number(parts[2]);
            const clanKey = parts[3];
            const label = clanKey === "neutral" ? null : clanLabel(clanKey);
            ...
            await sendScreen(ctx, `خاندان این Region روی ${label || "Neutral / بی‌طرف"} تنظیم شد.`);
            return;
        }
        // Spot جدید
        if (data.startsWith("admin:addspot:")) {
            ...
            await sendScreen(ctx, "نام Spot جدید برای این Region را بفرست.\n" +
        ...
        if (data.startsWith("admin:addedge:")) {
            ...
            if (error || !spots || spots.length === 0) {
                await sendScreen(ctx, "برای این Region هنوز Spotی تعریف نشده که بتوان مسیری ساخت.");
                return;
            }
            const kb = new grammy_1.InlineKeyboard();
            for (const s of spots) {
        ...
        if (data.startsWith("admin:edge_from:")) {
            ...
            await sendScreen(ctx, "خاندان/بی‌طرف/همه برای Region مقصد را انتخاب کن:", kb);
            return;
        }
        ...
        if (data.startsWith("admin:edge_destclan:")) {
            ...
            for (const r of regions) {
                const name = r.title || `Region #${r.id}`;
                kb.text(name, `admin:edge_destreg:${fromRegionId}:${fromSpotId}:${r.id}`).row();
            }
        ...
        if (data.startsWith("admin:edge_destreg:")) {
            ...
            const { data: spots, error } = await supabase
                ...
                .order("id", { ascending: true });
            if (error || !spots || spots.length === 0) {
                await sendScreen(ctx, "برای Region مقصد هنوز هیچ Spotی تعریف نشده.");
                return;
            }
            const kb = new grammy_1.InlineKeyboard();
            for (const s of spots) {
                kb.text(s.title, `admin:edge_to:${fromRegionId}:${fromSpotId}:${toRegionId}:${s.id}`).row();
            }
        ...
        if (data.startsWith("admin:edge_to:")) {
            ...
            const kb = new grammy_1.InlineKeyboard()
                .text("➡️ مسیر یک‌طرفه", `admin:edge_dir:${fromRegionId}:${fromSpotId}:${toRegionId}:${toSpotId}:one`)
                .row()
                .text("🔁 مسیر دوطرفه", `admin:edge_dir:${fromRegionId}:${fromSpotId}:${toRegionId}:${toSpotId}:two`);
            await sendScreen(ctx, "این مسیر یک‌طرفه باشد یا دوطرفه؟\n\n" +
                "یک‌طرفه: فقط از مبدأ به مقصد.\n" +
                "دوطرفه: هم از مبدأ به مقصد، هم از مقصد به مبدأ.", kb);
            return;
        }
        ...
        if (data.startsWith("admin:edge_dir:")) {
            ...
            await sendScreen(ctx, "مدت زمان سفر بین این دو Spot را به ثانیه بفرست.\n" +
                "مثال: 600");
            return;
        }
        ...
        if (data.startsWith("admin:listedges:")) {
            ...
            for (const e of edges) {
                const inRegion = spotIds.includes(e.from_spot_id) ||
                    spotIds.includes(e.to_spot_id);
                if (!inRegion)
                    continue;
                const fromSpot = spotMap.get(e.from_spot_id);
                ...
                text += `#${e.id} — ${lockIcon} ${(fromSpot === null || fromSpot === void 0 ? void 0 : fromSpot.title) || e.from_spot_id} → ${(toSpot === null || toSpot === void 0 ? void 0 : toSpot.title) || e.to_spot_id} (${e.travel_seconds}ث)\n`;
                count++;
            }
            if (count === 0) {
                await sendScreen(ctx, "هیچ Edgeی که به Spotهای این Region وصل باشد پیدا نشد.");
                return;
            }
            await sendScreen(ctx, text);
            return;
        ...
        if (data.startsWith("admin:delspot:")) {
            ...
            const { data: spots, error } = await supabase
                ...
                .order("id", { ascending: true });
            if (error || !spots || spots.length === 0) {
                await sendScreen(ctx, "برای این Region Spotی وجود ندارد که حذف شود.");
                return;
            }
            const kb = new grammy_1.InlineKeyboard();
            for (const s of spots) {
        ...
        if (data.startsWith("admin:delspot2:")) {
            ...
            await sendScreen(ctx, "Spot انتخاب‌شده حذف شد ✅\n" +
        ...
        if (data.startsWith("admin:deledge:")) {
            ...
            for (const e of edges) {
                ...
                const fromSpot = spotMap.get(e.from_spot_id);
                const toSpot = spotMap.get(e.to_spot_id);
                const label = `${(fromSpot === null || fromSpot === void 0 ? void 0 : fromSpot.title) || e.from_spot_id} → ${(toSpot === null || toSpot === void 0 ? void 0 : toSpot.title) || e.to_spot_id} (${e.travel_seconds}ث)`;
                kb.text(label, `admin:deledge2:${e.id}`).row();
                count++;
            }
            if (count === 0) {
                await sendScreen(ctx, "هیچ Edgeی مرتبط با این Region برای حذف پیدا نشد.");
                return;
            }
