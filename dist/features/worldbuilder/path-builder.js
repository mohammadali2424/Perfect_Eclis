...
exports.registerPathBuilderFeature = registerPathBuilderFeature;
...
function setWizard(ctx, state) {
    if (!ctx.from)
        return;
    if (state)
        wizardByUser.set(ctx.from.id, state);
    else
        wizardByUser.delete(ctx.from.id);
}
function buildModesKeyboard(state) {
    ...
    kb
        ...
        .row()
        .text(`حمل و نقل ${state.allowTransport ? "✅" : "❌"}`, "pb:mode:transport")
        .row()
        .text(`بلاک مونت ${state.blockMount ? "✅" : "❌"}`, "pb:mode:blockmount")
        .row()
...
function registerPathBuilderFeature(bot) {
    bot.hears("ساخت مسیر", async (ctx) => {
        ...
        await ctx.api.sendMessage(ctx.from.id, `🧵 شروع ساخت مسیر جدید برای Region: «${region.title}»\n\n` +
    ...
    bot.callbackQuery(/^pb:kind:(same|other)$/, async (ctx) => {
        ...
        if (kind === "same") {
            ...
            if (error || !spots || spots.length === 0) {
                await ctx.editMessageText("برای این Region هیچ Spotی ثبت نشده.\n" +
            ...
            await ctx.editMessageText("📍 مبدأ را انتخاب کن (یکی از Spotهای همین Region):", { reply_markup: kb });
        }
        else {
            ...
            for (const r of regions) {
                const label = r.id === wiz.fromRegionId
                    ? `⭐ ${r.title} (همین Region)`
                    : r.title;
    ...
    bot.callbackQuery(/^pb:targetRegion:(\d+)$/, async (ctx) => {
        ...
        if (error || !fromSpots || fromSpots.length === 0) {
            await ctx.editMessageText("در Region مبدأ هیچ Spotی ثبت نشده.\n" + "اول Spot بساز.");
            setWizard(ctx, null);
            return;
        }
        const kb = new grammy_1.InlineKeyboard();
        for (const s of fromSpots) {
            kb.text(s.title, `pb:fromSpot:${s.id}`).row();
        }
        kb.text("❌ لغو", "pb:cancel");
        await ctx.editMessageText("📍 مبدأ را انتخاب کن (Spot در Region مبدأ):", { reply_markup: kb });
    });
    bot.callbackQuery(/^pb:fromSpot:(\d+)$/, async (ctx) => {
        ...
        if (error || !toSpots || toSpots.length === 0) {
            await ctx.editMessageText("در Region مقصد هیچ Spotی ثبت نشده.\n" +
        ...
        kb.text("❌ لغو", "pb:cancel");
        await ctx.editMessageText("🎯 حالا مقصد را انتخاب کن (Spot در Region مقصد):", { reply_markup: kb });
    });
    bot.callbackQuery(/^pb:toSpot:(\d+)$/, async (ctx) => {
        ...
        wiz.step = "askTime";
        setWizard(ctx, wiz);
        await ctx.editMessageText("⏳ زمان سفر پیاده را مشخص کن.\n\n" +
            "یک عدد به *دقیقه* بفرست (مثلاً 10 یعنی حدوداً ۱۰ دقیقه).", { parse_mode: "Markdown" });
    });
    bot.on("message:text", async (ctx, next) => {
        var _a;
        if (!ctx.from || ctx.from.id !== MASTER_ID || ((_a = ctx.chat) === null || _a === void 0 ? void 0 : _a.type) !== "private") {
            return next();
        }
        const wiz = getWizard(ctx);
        ...
        const kb = new grammy_1.InlineKeyboard()
            ...
            .row()
            .text("❌ لغو", "pb:cancel");
        await ctx.reply(`⏳ زمان پیاده: حدود ${mins} دقیقه.\n` +
            `زمان رانندگی فعلی: ~${Math.round(mins / 2)} دقیقه.\n\n` +
            "جهت مسیر را انتخاب کن:", { reply_markup: kb });
    });
    bot.callbackQuery(/^pb:dir:(forward|backward|both)$/, async (ctx) => {
        ...
        setWizard(ctx, wiz);
        await ctx.editMessageText("نوع حرکت‌های مجاز روی این مسیر را انتخاب کن.\n\n" +
            "هر دکمه را بزن تا تیکش عوض شود، بعد دکمه‌ی ثبت را بزن.", { reply_markup: buildModesKeyboard(wiz) });
    });
    bot.callbackQuery(/^pb:mode:(walk|drive|transport|blockmount)$/, async (ctx) => {
        ...
        if (mode === "walk")
            wiz.allowWalk = !wiz.allowWalk;
        if (mode === "drive")
            wiz.allowDrive = !wiz.allowDrive;
        if (mode === "transport")
            wiz.allowTransport = !wiz.allowTransport;
        if (mode === "blockmount")
            wiz.blockMount = !wiz.blockMount;
        setWizard(ctx, wiz);
        try {
    ...
    bot.callbackQuery(/^pb:save:(once|again)$/, async (ctx) => {
        ...
        const base = {
            ...
            drive_seconds: wiz.allowDrive ? (_a = wiz.driveSeconds) !== null && _a !== void 0 ? _a : null : null,
        ...
        if (mode === "once") {
            setWizard(ctx, null);
            await ctx.editMessageText("✅ مسیر(های) جدید ثبت شد.\n" +
            ...
            const { data: toSpots, error } = await supabase
                ...
                .order("id", { ascending: true });
            if (error || !toSpots || toSpots.length === 0) {
                await ctx.editMessageText("مسیر ثبت شد، اما Spot دیگری در Region مقصد وجود ندارد.");
                setWizard(ctx, null);
                return;
            }
            const kb = new grammy_1.InlineKeyboard();
            ...
            kb.text("❌ لغو", "pb:cancel");
            await ctx.editMessageText("✅ مسیر ثبت شد.\n" +
                "برای ساخت مسیر بعدی، مقصد جدید را انتخاب کن:", { reply_markup: kb });
        }
    });
}
