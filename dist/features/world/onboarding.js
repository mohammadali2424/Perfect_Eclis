...
exports.registerOnboardingFeature = registerOnboardingFeature;
...
function clanKeyboardForRegister() {
...
async function sendMainMenuToUser(ctx, userId) {
    const api = ctx ? ctx.api : null;
    if (!api)
        return;
    try {
        await api.sendMessage(userId, "از این لحظه می‌توانی از منوی مسیر و نقشه استفاده کنی.", {
...
function registerOnboardingFeature(bot) {
    ...
    bot.command("start", async (ctx) => {
        ...
        const { data: char, error } = await supabase
            ...
            .eq("tg_id", tgId)
            .maybeSingle();
        let text = "به نقشه‌ی زندهٔ اکلیس خوش آمدی.\n" +
    ...
    bot.hears("ثبت من", async (ctx) => {
        ...
        if (char === null || char === void 0 ? void 0 : char.is_approved) {
            await ctx.reply("تو قبلاً در اکلیس ثبت شده‌ای.\n" +
                "از منوی زیر برای دیدن مسیرها و نقشه استفاده کن.", { reply_markup: mainMenuKeyboard() });
            return;
        }
        await ctx.reply("برای ثبت تو در جهان اکلیس، ابتدا خاندان خودت را انتخاب کن:", {
    ...
    bot.on("callback_query:data", async (ctx, next) => {
        ...
        if (data.startsWith("regclan:")) {
            ...
            await ctx.reply("درخواست ثبت‌نامت ثبت شد.\n" +
            ...
            try {
                const kb = new grammy_1.InlineKeyboard()
                    ...
                    .text("❌ رد", `reject:${tgId}`);
                const text = "یک درخواست ثبت‌نام جدید:\n\n" +
                    `کاربر: ${name}\n` +
                    `tg_id: ${tgId}\n` +
                    `خاندان: ${clan}\n\n` +
                    "آیا تایید می‌کنی؟";
        ...
        if (data.startsWith("approve:") || data.startsWith("reject:")) {
            ...
            if (action === "approve") {
                ...
                try {
                    await ctx.editMessageText((((_b = ctx.callbackQuery.message) === null || _b === void 0 ? void 0 : _b.text) || "") + "\n\n✅ تایید شد.");
                }
                catch (e) {
                    // مهم نیست اگر نشد
                }
            ...
            if (action === "reject") {
                ...
                try {
                    await ctx.editMessageText((((_c = ctx.callbackQuery.message) === null || _c === void 0 ? void 0 : _c.text) || "") + "\n\n❌ رد شد.");
                }
                catch (_e) { }
                // پیام محترمانه به خود پلیر
                try {
                    await ctx.api.sendMessage(targetId, "درخواست ثبت‌نامت در اکلیس توسط ارباب رد شد.");
                }
                catch (_e) { }
                return;
            }
        ...
        if (data.startsWith("pview:")) {
            ...
            const kb = new grammy_1.InlineKeyboard().text("🗑 حذف از ربات", `pdel:${tgId}`);
