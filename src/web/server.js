  ...
  const server = app.listen(cfg.port, async ()=>{
    ...
    if(cfg.renderUrl){
      ...
      try{
        ...
        await bot.telegram.setWebhook(url,{ allowed_updates:['message','callback_query','chat_join_request','my_chat_member','chat_member'] });
        console.log('✅ Webhook:',url);
      }catch(e){ console.log('Webhook warn:', e.message); }
      setInterval(()=>axios.head(`${cfg.renderUrl}/ping`).catch(()=>{}), 13*60*1000+59*1000);
    } else {
      try{
        await bot.telegram.deleteWebhook({ drop_pending_updates:true });
