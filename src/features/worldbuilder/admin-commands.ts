...
function isFromMasterInGroup(ctx: MyContext): boolean {
  if (!ctx.from) return false;
  if (ctx.from.id !== MASTER_ID) return false;
  if (!ctx.chat || ctx.chat.type === "private") return false;
  return true;
}

// گرفتن Region بر اساس chat_id گروه
async function getRegionForChat(ctx: MyContext) {
  const { supabase } = ctx.services;

  if (!ctx.chat) return { region: null, errorText: "چتی پیدا نشد." };
  ...
  return { region, errorText: null as string | null };
}
...
export function registerWorldAdminCommands(bot: Bot<MyContext>): void {
  ...
  bot.hears("ساخت منطقه", async (ctx) => {
    ...
    if (!region) {
      await sendToMaster(
        ctx,
        "دستور «ساخت منطقه» در گروهی زده شد که Region آن مشخص نیست.\n\n" +
          (errorText || "")
      );
      return;
    }
    ...
    const kb = new InlineKeyboard().text(
      "📍 ساخت منطقه (Spot) جدید",
      `admin:addspot:${region.id}`
    );

    const clanText =
      region.clan_name ? `خاندان: ${region.clan_name}\n` : "خاندان: نامشخص\n";
    ...
    await sendToMaster(
      ...
      "🧱 ساخت منطقه جدید\n" +
        ...
        "بعد از زدن دکمه، نام منطقه ازت پرسیده می‌شود.",
      kb
    );
  });
...
// 🗑 حذف مسیر (Edge)
  bot.hears("حذف مسیر", async (ctx) => {
    ...
    if (!region) {
      await sendToMaster(
        ctx,
        "دستور «حذف مسیر» در گروهی زده شد که Region آن مشخص نیست.\n\n" +
          (errorText || "")
      );
      return;
    }
    ...
    const kb = new InlineKeyboard().text(
      "🗑 انتخاب مسیر برای حذف",
      `admin:deledge:${region.id}`
    );
    ...
    await sendToMaster(
      ...
      "🗑 حذف مسیر\n" +
        ...
        `Region: ${region.title}\n` +
        `region_id: ${region.id}\n\n` +
        "با دکمه‌ی زیر لیست Edgeهای مرتبط با این Region را می‌بینی و می‌توانی هرکدام را حذف کنی.",
      kb
    );
  });
  ...
  bot.hears("حذف منطقه", async (ctx) => {
    ...
    if (!region) {
      await sendToMaster(
        ctx,
        "دستور «حذف منطقه» در گروهی زده شد که Region آن مشخص نیست.\n\n" +
          (errorText || "")
      );
      return;
    }
    ...
    const kb = new InlineKeyboard().text(
      "🗑 انتخاب منطقه برای حذف",
      `admin:delspot:${region.id}`
    );
    ...
    await sendToMaster(
      ...
      "🗑 حذف منطقه\n" +
        ...
        "با حذف یک Spot، تمام Edgeهای متصل به آن هم به‌صورت خودکار (به‌خاطر FK) حذف می‌شوند.",
      kb
    );
  });
  ...
  bot.hears("تنظیمات ریجن", async (ctx) => {
    ...
    if (!region) {
      await sendToMaster(
        ctx,
        "دستور «تنظیمات ریجن» در گروهی زده شد که Region آن مشخص نیست.\n\n" +
          (errorText || "")
      );
      return;
    }
    ...
    const kb = new InlineKeyboard().text(
      "⚙️ باز کردن پنل Region",
      `admin:openregion:${region.id}`
    );

    const clanText =
      region.clan_name || region.clan_name === ""
    ...
    await sendToMaster(
      ...
      "⚙️ تنظیمات Region\n" +
        ...
        "ساخت/حذف Spot، ساخت/حذف Edge، قفل‌ها و غیره.",
      kb
    );
  });
  ...
  bot.hears("لیست مسیرها", async (ctx) => {
    ...
    if (!region) {
      await sendToMaster(
        ctx,
        "دستور «لیست مسیرها» در گروهی زده شد که Region آن مشخص نیست.\n\n" +
          (errorText || "")
      );
      return;
    }
    ...
    await sendToMaster(
      ...
      "📜 لیست مسیرها\n" +
        ...
        "خود پنل لیست، جزئیات را برایت می‌نویسد.",
      kb
    );
  });
  ...
  bot.hears("لیست مناطق", async (ctx) => {
    ...
    if (!region) {
      await sendToMaster(
        ctx,
        "دستور «لیست مناطق» در گروهی زده شد که Region آن مشخص نیست.\n\n" +
          (errorText || "")
      );
      return;
    }
    ...
    await sendToMaster(
      ...
      "📍 لیست مناطق\n" +
        ...
        "از همان پنل هم می‌توانی برای ویرایش/حذف استفاده کنی.",
      kb
    );
  });
}
