...
function register(bot){
  bot.action(/^g:([A-Za-z0-9_-]{6,18})$/i, async (ctx)=>{
    ...
    if(gtype==='sub'){
      ...
      return ctx.reply(`شما درحال حرکت هستی…\n\nمسیر شما به سمت «${destPage?.title||'مقصد'}» است.\nمدت مسیر: ${humanize(etaSec)}`, Markup.inlineKeyboard([[Markup.button.callback('❌ لغو حرکت',`c:${cancelTok.slice(2)}`)]]));
    }
    ...
    const destPage=await getPageById(g.to_page_id); const cancelTok=putCancelToken({ move_id:moveId, from_chat_id:`${g.from_chat_id}` });
    await ctx.answerCbQuery('بلیت در PV ارسال شد').catch(()=>{});
    return ctx.telegram.sendMessage(uid,`شما درحال حرکت هستی…\n\nمسیر شما به سمت «${destPage?.title||'مقصد'}» است.\nمدت مسیر: ${humanize(etaSec)}\n\nوقتی به زمان مقرر رسیدی، روی دکمه بزن:`, 
      Markup.inlineKeyboard([[Markup.button.url('ورود به مقصد', pooled.invite_link)],[Markup.button.callback('❌ لغو حرکت',`c:${cancelTok.slice(2)}`)]]));
  });
}
