...
function register(bot){
  bot.action(/^c:([A-Za-z0-9_-]{6,18})$/i, async (ctx)=>{
    const tok=`c:${ctx.match[1]}`; const payload=require('../../utils/cache').cbMap.get(tok); if(!payload) return ctx.answerCbQuery('منقضی').catch(()=>{});
    const { move_id, from_chat_id } = payload; const uid=ctx.from.id;
    const { data } = await supa.from('movements').select('move_id,user_id,departed_at,state').eq('move_id',move_id).eq('user_id',uid).eq('state','scheduled').maybeSingle();
