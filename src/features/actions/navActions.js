...
function register(bot){
  bot.action(/^pnav:(-?\d{6,20}):(.+)$/i, async (ctx)=>{
    ...
    try{ await ctx.editMessageText(view.text,view.kb);}catch{ await ctx.reply(view.text,view.kb);} await ctx.answerCbQuery().catch(()=>{});
  });
  ...
  bot.action('pmenu:eta', async (ctx)=>{
    ...
    const d=new Date(m.arrive_at).getTime()-Date.now(); if(d<=0) return ctx.answerCbQuery('به مقصد رسیدی (یا هر لحظه)').catch(()=>{});
