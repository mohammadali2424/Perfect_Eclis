// src/features/actions/microActions.js
...
function register(bot){
  bot.action(/^m:([A-Za-z0-9_-]{6,32})$/i, async (ctx)=>{
    if(ctx.chat?.type!=='private') return ctx.answerCbQuery().catch(()=>{});
    const tok=`m:${ctx.match[1]}`; const payload=getMicroToken(tok); if(!payload) return ctx.answerCbQuery('منقضی است').catch(()=>{});
    ...
    const announce=async()=>{
      if(chatId){
        try{ await safeSend(bot, chatId, `🎯 پلیر ${mention(uid)} ${String(v.text||'').replace(/^📜\s*/,'').trim()} شد`, { parse_mode: 'Markdown' }); }catch{}
      }
    };

    if(eta && eta>0){
      try{ await ctx.answerCbQuery(`⏳ ${eta} ثانیه...`); }catch{}
      setTimeout(async ()=>{
        try{ await ctx.editMessageText(v.text, v.kb);} catch { await ctx.reply(v.text, v.kb); }
        await announce();
      }, eta*1000);
    } else {
      try{ await ctx.editMessageText(v.text, v.kb);} catch { await ctx.reply(v.text, v.kb); }
