...
export function registerVehicleTravelFeature(bot: Bot<MyContext>) {
  ...
  if (wErr) console.error("flux:fuel wells error:", wErr);
  ...
  if (veh.current_region_id !== char.current_region_id || veh.current_spot_id !== char.current_spot_id) {
    await ctx.reply("برای سوخت‌گیری باید کنار چاه فلوکس و کنار وسیله باشی.");
    return;
  }

  // پر کردن باک (مثلاً 100)
  const { error: updErr } = await supabase
    .from("vehicles")
