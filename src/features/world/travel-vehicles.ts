...
import { computeFuelPurchase } from "../../services/economy/fuel";
...
async function sendVehicleScreen(
  ...
  const lastId = s.ui_last_message_id as number | undefined;
...
/** بررسی اینکه آیا در این نقطه چاه فلوکس هست یا نه (سازگار با هر امضایی از hasFluxWell) */
...
export async function showTransportMenu(ctx: MyContext) {
  ...
  if (char.current_region_id && char.current_spot_id) {
    ...
    hasFlux = await hasFluxHere(ctx, char.current_region_id, char.current_spot_id);
  }
...
async function showVehicleDetail(ctx: MyContext, vehicleId: number) {
  ...
  const kb = new InlineKeyboard();
  kb.text(locked ? "🔓 باز کردن برای مسافرها" : "🔒 بستن برای مسافرها", `veh:lock:${vehicle.id}`).row();
...
async function handleDriveVehicle(ctx: MyContext, vehicleId: number) {
  ...
  if (!ovErr && otherVehicles && otherVehicles.length > 0) {
    ...
    const { error: clearErr } = await supabase
      ...
      .in("id", otherIds);
    if (clearErr) console.error("handleDriveVehicle clear other vehicles error:", clearErr);
  }
  ...
  if (charErr) console.error("handleDriveVehicle char update error:", charErr);
...
async function handleLeaveVehicle(ctx: MyContext, vehicleId: number) {
  ...
  if (vehicle.current_driver_char_id === char.id) {
    ...
    if (updCharErr) console.error("handleLeaveVehicle driver char update error:", updCharErr);
  ...
  if (updCharErr2) console.error("handleLeaveVehicle passenger char update error:", updCharErr2);

  await ctx.reply(`🚶 از ${vehicle.display_name ?? vehicle.title ?? "وسیله"} پیاده شدی.`);
}

/** قفل/باز کردن اجازه‌ی سوار شدن مسافران توسط صاحب وسیله */
async function toggleVehiclePassengerLock(ctx: MyContext, vehicleId: number) {
  const { supabase } = ctx.services;
  const { char, errorText } = await getCharacterByTg(ctx);
  if (!char) {
    await ctx.reply(errorText ?? "پرونده‌ات را پیدا نکردم.");
...
async function showRideMenu(ctx: MyContext) {
  ...
  if (alreadyPassenger) {
    await ctx.reply("الان به عنوان مسافر داخل یک وسیله‌ای.\nاگر بخواهی پیاده شوی باید از منوی وضعیت/پیاده شدن استفاده کنی.");
    return;
  }
  ...
  if (!vehicles || vehicles.length === 0) {
    const kb = new InlineKeyboard().text("⬅️ بازگشت به حمل‌ونقل", "trans:menu");
...
async function sendRideRequestToDriver(ctx: MyContext, vehicleId: number, passengerCharId: number) {
  const { supabase } = ctx.services;
...
async function handleRideDecision(
  ...
  await ctx.reply(`مسافر ${passenger.char_name} را سوار ${vehicle.title ?? "وسیله"} کردی.`);

  if (passenger.tg_id) {
    try {
      await ctx.api.sendMessage(passenger.tg_id, `🚕 راننده تو را سوار ${vehicle.title ?? "وسیله"} کرد.`);
    } catch (e) {
...
export async function moveVehicleWithPassengers(
  ...
  if (fromSpotId && toSpotId) {
    ...
    if (edgeErr) console.error("moveVehicleWithPassengers edge error:", edgeErr);
    else if (edge) driveSeconds = edge.drive_seconds ?? edge.travel_seconds ?? 0;
  }
  ...
  if (updVehErr) console.error("moveVehicleWithPassengers vehicle update error:", updVehErr);

  await logVehicleMove(ctx, vehicleId, fromSpotId, toSpotId, "drive");

  const { driverId, passengerIds } = await getVehicleLoad(ctx, vehicleId);
  const involvedCharIds = [...(driverId ? [driverId] : []), ...passengerIds];
  ...
  if (updErr) console.error("moveVehicleWithPassengers char update error:", updErr);
}
...
export function registerVehicleTravelFeature(bot: Bot<MyContext>) {
  ...
  // بعضی منوها در travel.ts از ride:home می‌سازن
  ...
  return;
    ...
    await ctx.reply(
      `⛽ سوخت‌گیری برای «${veh.title ?? "وسیله"}»\n` +
        ...
        `قیمت: هر 1٪ = ${FLUX_PRICE_PER_PERCENT} سولن`,
      { reply_markup: kb }
    );
  });
