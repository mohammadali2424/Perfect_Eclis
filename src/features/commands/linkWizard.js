...
//
// ویزارد لینک مرحله‌به‌مرحله (مثل قبل):
// - اگر در «گروه» صدا زده شود: پیام دستور پاک می‌شود و ویزارد در PV با «همین گروه = مبدا» شروع می‌شود.
// - اگر در «PV» صدا زده شود: از کاربر می‌خواهد گروه مبدا را فوروارد/آیدی بدهد.
// مراحل:
//   1) انتخاب/ساخت صفحه مبدا + توضیح/اینفو صفحه جدید
//   2) نوع مسیر: اصلی/فرعی/ریزمسیر
//   3) برای اصلی: تعیین گروه مقصد + انتخاب/ساخت صفحه مقصد
//      برای فرعی: انتخاب/ساخت صفحه مقصد در همان گروه
//      برای ریزمسیر: مقصد همان صفحه مبدا است
//   4) برچسب، ایموجی اختیاری، مدت مسیر (حداقل 10s)، توضیح/اینفو مسیر اختیاری
//   5) مرور نهایی + ذخیره
//   6) پیشنهاد ساخت مسیر برگشت (با برچسب/مدت قابل تنظیم)
...
function pageButtons(pages, purpose = 'from') {
  ...
  rows.push([Markup.button.callback('➕ ساخت صفحه جدید', `lw:page:${purpose}:new`)]);
...
async function askToPage(ctx, chat_id, type) {
  ...
  const head = type === 'main' ? 'گام ۵) صفحهٔ مقصد:' : 'گام ۴) صفحهٔ مقصد (در همین گروه):';
...
async function summary(ctx, st) {
  ...
  const lines = [
    ...
    `• مقصد: ${toChat} | صفحه: ${st.type === 'micro' ? st.from_page_id : (st.to_page_id || st.from_page_id)}`,
    `• برچسب: ${st.label}`,
    `• ایموجی: ${st.emoji || '-'}`,
  ...
  return ctx.reply(
    lines.join('\n'),
    Markup.inlineKeyboard([[Markup.button.callback('✅ ذخیره', 'lw:save')], [Markup.button.callback('❌ لغو', 'lw:cancel')]])
  );
}
async function askMakeReverse(ctx, st) {
  if (!(st.type === 'main' || st.type === 'sub')) return; // برای micro برگشت لازم نیست
  ...
  await ctx.reply(
    'مسیر ذخیره شد. مسیر برگشت هم بسازم؟',
...
function register(bot, config) {
  // اگر در گروه اجرا شد → پیام پاک + شروع در PV با پیش‌فرض مبدا = همین گروه
  bot.command('link_wizard', async (ctx) => {
    if (ctx.chat?.type === 'group' || ctx.chat?.type === 'supergroup') {
      ...
      try {
        ...
        // فقط اگر PV بسته باشد، پیام کوتاه راهنما بده و پاک کن
        const me = ctx.botInfo?.username;
        if (!me) return;
        const url = `https://t.me/${me}`;
        const m = await ctx.reply(
          'برای شروع ویزارد، لطفاً یک‌بار به پی‌وی من برو و /start بزن.',
          Markup.inlineKeyboard([[Markup.button.url('📥 باز کردن پی‌وی ربات', url)]])
        );
  ...
  bot.action(/^lw:type:(main|sub|micro)$/i, async (ctx) => {
    ...
    if (!s || s.stage !== 'type') return ctx.answerCbQuery().catch(() => {});
  ...
  // انتخاب صفحه (from/to) یا ساخت صفحه‌ی جدید
  bot.action(/^lw:page:(from|to):(new|\d+)$/i, async (ctx) => {
    const s = S(ctx.from.id);
    if (!s) return ctx.answerCbQuery().catch(() => {});
  ...
  bot.action(/^lw:save$/i, async (ctx) => {
    const s = S(ctx.from.id);
    if (!s || s.stage !== 'confirm') return ctx.answerCbQuery().catch(() => {});
    try {
      const gate = {
      ...
      if (err) {
        ...
        await askMakeReverse(ctx, s); // پیشنهاد ساخت برگشت
      }
    } catch {
      await ctx.reply('❌ خطا در ذخیره');
    }
    try { await ctx.answerCbQuery('ذخیره شد'); } catch {}
    // سشن را موقتاً نگه می‌داریم برای برگشت
  ...
  bot.action(/^lw:rev_yes:([A-Za-z0-9+/=]+)$/i, async (ctx) => {
    ...
    if (!base) return ctx.answerCbQuery().catch(() => {});
    let rev;
    try { rev = JSON.parse(Buffer.from(ctx.match[1], 'base64').toString('utf8')); } catch { rev = null; }
