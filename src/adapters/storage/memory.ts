...
export class MemoryXpLedgerRepo implements XpLedgerRepo {
  ...
  async listByChat(chatId: ChatId, opts?: { sinceIso?: string }): Promise<XpLedgerEntry[]> {
    const since = opts?.sinceIso ? new Date(opts.sinceIso).getTime() : -Infinity;
    return this.entries.filter((e) => e.chatId === chatId && new Date(e.createdAt).getTime() >= since);
  }
