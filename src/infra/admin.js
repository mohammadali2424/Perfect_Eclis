...
async function isBotAdmin(bot, chatId, meId) {
  const k = `admin:${chatId}`;
  const c = cache.get(k);
  if (c !== undefined) return c;
  try {
    const me = await bot.telegram.getChatMember(chatId, meId);
    const ok = !!me && ['administrator','creator'].includes(me.status);
    cache.set(k, ok, 600);
    return ok;
  } catch {
...
async function softKick(bot, chatId, userId, meId) {
  try {
    ...
    try {
      ...
      if (!m || ['left','kicked','creator'].includes(m.status)) return true;
    } catch {}
...
async function kickOthers(bot, keepChatId, userId, meId, registered) {
  for (const r of registered) {
    const cid = `${r.chat_id}`;
    if (cid === `${keepChatId}`) continue;
    await softKick(bot, cid, userId, meId);
  }
}
