// src/infra/invitePool.js
// Pool ساده برای ساخت لینک‌های join-request با عمر 5 دقیقه.
// اگر لینک سالم داریم، همونو می‌دیم؛ اگر نه، جدید می‌سازیم.
...
async function getPooledJoinRequestLink(bot, toChatId) {
  ...
  // اگر هنوز حداقل 45 ثانیه عمر دارد، همان را برگردان
...
function sweep() {
  const now = Date.now();
  for (const [k, v] of invitePool.entries()) {
    if (v.expireAtTs <= now) invitePool.delete(k);
  }
}
