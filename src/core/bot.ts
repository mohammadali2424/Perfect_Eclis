...
// فقط برای اینکه مطمئن باشیم config خونده شده
...
// =========================
//  تست اینکه این فایل واقعا لود شده
// =========================
bot.command("debug_alive", async (ctx) => {
  await ctx.reply("✅ Core bot زنده است و bot.ts فعلی در حال اجراست.");
});
...
// تست اینکه منطق ثبت‌نام وصل شده
bot.command("debug_reg", async (ctx) => {
  await ctx.reply("✅ ماژول ثبت‌نام (داخل bot.ts) فعاله.");
});
