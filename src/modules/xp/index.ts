  ...
  registry.register({
    ...
    handler: async (ctx, { uow, logger }) => {
      ...
      if (sub === 'report' || sub === 'گزارش') {
        ...
        const sum = entries.reduce((a: number, e) => a + e.amount, 0);
        await ctx.reply(
          [`گزارش XP این چت (آخرین ${entries.length} رویداد):`, `جمع: ${sum}`, '', ...entries.map((e: any) => `• ${e.playerId}: ${e.amount > 0 ? '+' : ''}${e.amount} — ${e.reason}`)].join('\n')
        );
        return;
      }
