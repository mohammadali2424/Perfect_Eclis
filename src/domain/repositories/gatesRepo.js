...
async function insertGate(g){ const {error}=await supa.from('gates').insert(g); if(!error){ cache.del(`gates:from:${g.from_page_id}`); if(g.type==='main') cache.del(`gates:from:${g.to_page_id}`);} return error; }
async function updateGate(id,fields){ await supa.from('gates').update(fields).eq('id',id); }
